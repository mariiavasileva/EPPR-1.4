...
std::vector<double>& MatrixGSL::getData() {
	return matrixObject->getData();
}
int MatrixGSL::getRows() {
	return matrixObject->getRows();
}
int MatrixGSL::getColumns() {
	return matrixObject->getCols();
}
int MatrixGSL::getLength() {
	return matrixObject->getLength();
}
double MatrixGSL::get(int i, int j) {
	return matrixObject->get(i,j);
}
void MatrixGSL::put(int i, int j, double v) {
	matrixObject->set(i, j, v);
}
...
std::shared_ptr<IMatrix> MatrixGSL::repmat(int rowMult, int columnMult) {
	DoubleMatrix* result = new DoubleMatrix(getRows() * rowMult, getColumns() * columnMult);
...
double MatrixGSL::max() {
	...
	if (matrixObject->getLength() == 0) return -std::numeric_limits<double>::infinity();
...
{
	this->matrixObject = std::make_shared<DoubleMatrix>((size_t)n, (size_t)m, val);
}


DoubleMatrix* MatrixGSL::getMatrixPtr(void)
{
	return matrixObject.get();
}


int MatrixGSL::write(hid_t file, std::string dataset_name)
{		
	return matrixObject->write(file, dataset_name);
}


int MatrixGSL::read(hid_t file, std::string dataset_name)
{
	return matrixObject->read(file, dataset_name);
}

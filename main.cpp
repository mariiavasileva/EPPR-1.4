...
using namespace std;
double beta = 2.0;
// 0.95000002 conf beta=1.95996423295
// 0.99000014 conf beta=2.57583444037
// 0.99900008 conf beta=3.29054969052
// 0.99990007 conf beta=3.89078435285
// 0.99999000 conf beta=4.41728191994
...
std::string file_source = "source.h5";
std::string file_training = "training.h5";
std::string file_xtpoints = "xtpoints.h5";
std::string file_result = "result.h5";

hid_t open_hdf(std::string name) {
...
void test_prepare_data(hid_t file) {
	...
	DataStorage::saveMatrix(file, string("/xpoints"), x);
	DataStorage::saveMatrix(file, string("/ypoints"), y);
}

void test_training(hid_t file_source, hid_t file_training) {
	vector<vector<double> > x;
	vector<double> y;
	vector<vector<double> > x_range;
	DataStorage::loadMatrix(file_source, string("/xpoints"), x);
	DataStorage::loadMatrix(file_source, string("/ypoints"), y);

	x_range.resize(2);
	x_range[0].resize(x[0].size());
	size_t last = x.size() - 1;
	x_range[1].resize(x[last].size());
	std::copy(x[0].begin(), x[0].end(), x_range[0].begin());
	std::copy(x[last].begin(), x[last].end(), x_range[1].begin());

	shared_ptr<GpDataset> dataset = make_shared<GpDataset>(x, y);
	DataStorage::saveMatrix(file_training, "/xpoints", x);
	DataStorage::saveMatrix(file_training, "/ypoints", y);
	...
	approx->gp->save_state(file_training);
	DataStorage::saveMatrix(file_training, "/range", x_range);
}

void test_prepare_test_data(hid_t file_xt) {
	...
	DataStorage::saveMatrix(file_xt, string("/xtpoints"), xt);
}

void test_get_result(hid_t file_training, hid_t file_xt, hid_t file_result) {
	vector<vector<double> > *xt;
	xt = new std::vector<std::vector<double> >();
	DataStorage::loadMatrix(file_xt, string("/xtpoints"), *xt);
	
	vector<vector<double> > x;
	vector<double> y;
	DataStorage::loadMatrix(file_training, "/xpoints", x);
	DataStorage::loadMatrix(file_training, "/ypoints", y);
	shared_ptr<GpDataset> dataset = make_shared<GpDataset>(x, y);
	
	shared_ptr<SmmcOptions> options = make_shared<SmmcOptions>(ncolumns + 1);
	std::shared_ptr<GPEP> gp = std::make_shared<GPEP>(options->getKernelGP());
	gp->setTrainingSet(dataset);
	gp->setScale(options->getSimulationRuns());
	gp->setCovarianceCorrection(options->getCovarianceCorrection());

	gp->restore_state(file_training);
	std::shared_ptr<AnalyticApproximation> approx = std::make_shared<AnalyticApproximation>(gp);
	...
	DataStorage::saveMatrix(file_result, std::string("ClassProbabilities"), result->getClassProbabilities());
	DataStorage::saveMatrix(file_result, std::string("LowerBound"), result->getLowerBound(beta));
	DataStorage::saveMatrix(file_result, std::string("UpperBound"), result->getUpperBound(beta));
...
void test_storage() {
	hid_t source = open_hdf(file_source);
	hid_t training = open_hdf(file_training);
	hid_t xtpoints = open_hdf(file_xtpoints);
	hid_t result = open_hdf(file_result);

	test_prepare_data(source);
	test_training(source, training);
	test_prepare_test_data(xtpoints);
	test_get_result(training, xtpoints, result);

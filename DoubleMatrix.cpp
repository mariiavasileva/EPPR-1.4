...
DoubleMatrix* DoubleMatrix::solve(DoubleMatrix* A, DoubleMatrix* B)
{
	...
	int signum = 0;
	unsigned long dim = (unsigned long)A->matrix->size1;
...
int DoubleMatrix::write(hid_t file, std::string dataset_name)
{
	SHDF shdf;
	shdf.cols = (size_t)getCols();
	shdf.rows = (size_t)getRows();
	double* buf = new double[shdf.rows * shdf.cols];
	shdf.data = buf;
	double* data = buf;
	for (size_t r = 0; r < shdf.rows; r++)
		for (size_t c = 0; c < shdf.cols; c++) {
			*data++ = get((int)r, (int)c);
		}
	DataStorage::write(file, dataset_name, &shdf);
	delete[] buf;
	return 0;
}


int DoubleMatrix::read(hid_t file, std::string dataset_name)
{
	SHDF shdf;
	DataStorage::read(file, dataset_name, &shdf);
	matrix = gsl_matrix_alloc(shdf.rows, shdf.cols);
	double* data_row = matrix->data;
	for (size_t i = 0; i < shdf.rows; ++i) {
		double *data = data_row;
		double* source = shdf.data + i*shdf.cols;
		memcpy(data, source, sizeof(double) * shdf.cols);
		data_row += matrix->tda;
	}
	delete[] shdf.data;
	return 0;
